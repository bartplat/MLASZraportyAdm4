...
params:
  plikZObiektami: ../../data/wskaznikiAdm4.RData
  obiektWskazniki: szk
...
W porównaniu z raportami z wcześniejszych edycji monitoringu karier, zwiększono liczbę prezentowanych statusów, opisujących sytuację edukacyjno-zawodową absolwentów. Status „Brak nauki i brak pracy” rozdzielono na „Bezrobocie” oraz „Brak danych o aktywności”. Poniższa tabela objaśnia konstrukcję poszczególnych statusów.
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "%.\n\n",
    "Naukę w systemie edukacji formalnej z zatrudnieniem na podstawie umowy o pracę lub z wykonywaniem własnej działalności gospodarczej (samozatrudnieniem)^[W przypadku uczniów i studentów do 26. roku życia nie są dostępne dane dotyczące ewentualnego wykonywania pracy na podstawie umów cywilnoprawnych.] łączyło ",
    ...
    "% absolwentów Państwa szkoły policealnej pracowało^[Na podstawie umowy o pracę, umowy zlecenia lub prowadząc własną działalność gospodarczą.], ale nie kontynuowało nauki. Do grupy pracujących, ale nie kontynuujących edukacji formalnej należało ",
    ...
    params$rok,
    " roku ",
    round(szk$S3_12[[1]]$bezrob * 100),
    "% było zarejestrowanych w urzędzie pracy jako osoby bezrobotne. W takiej samej sytuacji było ",
    round(god$S3_12[[1]]$bezrob * 100),
    "% absolwentów pozostałych szkół policealnych z ",
    powiaty_dop,
    ".\n\n",
    "W grudniu ",
    params$rok,
    " nie odnotowano żadnej aktywności związanej z pracą zawodową lub nauką (tj. wykonywania pracy, kontynuowania edukacji formalnej ani zarejestrowanego bezrobocia) w przypadku ",
    round(szk$S3_12[[1]]$neet * 100),
    "% absolwentów Państwa szkoły policealnej oraz ",
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  ...
  tab = tab_wykres(szk$S3_12[[1]],
                   ...
                   szk$typ_szk[[1]],
                   80) %>% 
    mutate(name = ordered(name, levels = c("Brak danych o aktywności",
                                           "Bezrobocie",
                                           "Tylko praca",
                                           "Nauka i praca",
                                           "Tylko nauka")))
  ...
  tabela = tab %>% 
    mutate(across(name,
                  ~ordered(name,
                           levels = c("Brak danych o aktywności",
                                      "Bezrobocie",
                                      "Tylko praca",
                                      "Nauka i praca",
                                      "Tylko nauka"),
    ...
    select(typ, tylko_ucz, ucz_prac, tylko_prac, bezrob, neet) %>% 
    ...
    for (i in tab_s3_zaw_check$typ) {
      ...
      if (nrow(tab_god) > 0) {
        ...
        tabela = rbind(tab_szk, tab_god) %>% 
          pivot_longer(cols = tylko_ucz:neet, names_to = "name") %>% 
          mutate(across(name,
                        ~case_when(
                          . %in% "tylko_ucz" ~ "Tylko nauka",
                          . %in% "ucz_prac" ~ "Nauka i praca",
                          . %in% "tylko_prac" ~ "Tylko praca",
                          . %in% "bezrob" ~ "Bezrobocie",
                          . %in% "neet" ~ "Brak danych o aktywności"))) %>% 
          mutate(lab = ifelse(round(value, 2) >= 0.06,
                              paste0(round(value * 100), "%"),
                              ""),
                 name = ordered(name, levels = c("Brak danych o aktywności",
                                                 "Bezrobocie",
                                                 "Tylko praca",
                                                 "Nauka i praca",
                                                 "Tylko nauka")))
...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  lip_sier = c(
    "lipcu" = round(szk$S3_07[[1]]$ucz_prac * 100) + round(szk$S3_07[[1]]$tylko_prac * 100),
    "sierpniu" = round(szk$S3_08[[1]]$ucz_prac * 100) + round(szk$S3_08[[1]]$tylko_prac * 100))
  ...
  cat(
    ...
    if (length(unique(lip_sier)) %in% 1) {
      paste0(round(szk$S3_07[[1]]$ucz_prac * 100), "%.\n\n")
    } else {
      paste0(
        "od ",
        min(lip_sier),
    ...
    params$rok,
    " roku odsetek absolwentów Państwa szkoły pracujących na podstawie umowy o pracę, samozatrudnionych, a także - w przypadku osób niekontynuujących nauki - pracujących na podstawie umowy zlecenia, wynosił łącznie ",
    ...
    " Bardziej szczegółowe wyniki, uwzględniające również to czy absolwenci się uczyli lub byli zarejestrowani jako osoby bezrobotne, przedstawia poniższy wykres.\n\n",
    sep = ""
  )
}
```
...
if (szk$E2_nauka_kontyn[[1]]$n >= progLiczebnosci) {
  ...
  if (sposoby != 0) {
    ...
    if (round(szk$E2_nauka_kontyn[[1]]$brak * 100) == 50) {
      cat(
        "\n\nWśród absolwentów Państwa szkoły z rocznika ",
        rocznik,
        " połowa absolwentów nie kontynuowała kształcenia w grudniu ",
        params$rok,
        " roku.\n\n",
        sep = ""
      )
    ...
    cat("Żaden z absolwentów Państwa szkoły z rocznika ",
        ...
        " w grudniu ",
        params$rok,
        " roku nie kontynuował nauki w systemie edukacji formalnej.",
        sep = "")
  }
} else {
...
```{r}
if (any(!is.na(szk$liczebnosc_dziedziny[[1]]$odsetek))) {
  if (round(szk$E2_nauka_kontyn[[1]]$studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
    if (sum(szk$liczebnosc_dziedziny[[1]]$odsetek > 0, na.rm = TRUE) == 1) {
      cat(
    ...
    } else if (sum(szk$liczebnosc_dziedziny[[1]]$odsetek > 0, na.rm = TRUE) > 1 & sum(szk$liczebnosc_dziedziny[[1]]$odsetek %in% max(szk$liczebnosc_dziedziny[[1]]$odsetek)) == 1) {
      cat(
...
```{r}
if (any(!is.na(szk$liczebnosc_dyscypliny[[1]]$odsetek))) {
  if (round(szk$E2_nauka_kontyn[[1]]$studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
    if (sum(szk$liczebnosc_dyscypliny[[1]]$odsetek > 0, na.rm = TRUE) == 1) {
      cat(
    ...
    } else if (sum(szk$liczebnosc_dyscypliny[[1]]$odsetek > 0, na.rm = TRUE) > 1 & sum(szk$liczebnosc_dyscypliny[[1]]$odsetek %in% max(szk$liczebnosc_dyscypliny[[1]]$odsetek)) == 1) {
      cat(
...
## Kontynuowanie nauki na studiach w poszczególnych dyscyplinach w podziale na wyuczony zawód
...
if (round(szk$E2_nauka_kontyn[[1]]$studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
  ...
  if (length(szk$dyscypliny_zawody[[1]]) > 1 & l_col > 0) {
    cat(
      "W poniższej tabeli przedstawione zostały informacje o dyscyplinach, w których studiowali absolwenci Państwa szkoły, należący do poszczególnych grup (liczących co najmniej 10 osób) wyróżnionych ze względu na zawód, w którym się kształcili.")
    ...
    szk$dyscypliny_zawody[[1]] %>% 
      ...
      mutate(across(2:ncol(.),
                    ~paste0(round(. * 100), "\\%"))) %>% 
      mutate(across(1, ~wrapper(., width = 50))) %>% 
      kable(format = "latex",
            booktabs = TRUE,
            escape = FALSE,
            longtable = TRUE,
...
Wskaźnik, którego wartości prezentujemy w tej części raportu, pokazuje stosunek przychodów absolwentów Państwa szkoły z pracy etatowej^[Średnią (arytmetyczną) obliczono wyłącznie dla miesięcy, w których absolwenci pracowali. Nie uwzględniono przychodów przekraczających  kwotę rocznego ograniczenia podstawy wymiaru składek na ubezpieczenie emerytalne i rentowe (tj. 30 krotność prognozowanego wynagrodzenia brutto, czyli w `r params$rok` roku 208 050 zł), w miesiącach, w których to przekroczenie wystąpiło.] do przeciętnego miesięcznego wynagrodzenia^[Według danych publikowanych w [Banku Danych Lokalnych Głównego Urzędu Statystycznego](https://stat.gov.pl/metainformacje/slownik-pojec/pojecia-stosowane-w-statystyce-publicznej/693,pojecie.html?pdf=1)] osób mieszkających w tych samych powiatach. Dla poszczególnych absolwentów wartość wskaźnika równa 100% oznaczałaby, że ich średnie zarobki z pracy etatowej były takie same, jak przeciętne wynagrodzenie mieszkańców danego powiatu. 
...
```{r, out.width='100%'}
knitr::include_graphics("../boxplot_edycja4.png")
```
...
# 5. Bezrobocie rejestrowane^[Wyniki prezentowane w tym rozdziale opierają się na danych dotyczących absolwentów zarejestrowanych jako osoby bezrobotne, niezależnie od tego czy kontynuowały naukę. Natomiast w rozdziale 2 z kategorii absolwentów, którym przypisano status “Bezrobocie” wykluczono osoby, które zarazem kontynuowały edukację.] od września do grudnia `r params$rok` roku
...
# 6. Aneks: wyniki dla absolwentów szkół policealnych `r ifelse(woj$l_abs[[1]] < progLiczebnosci, paste0(""), paste0("z województwa i", "\u00A0"))`z`r paste0("\u00A0")`całego kraju
...
woj = woj %>% 
  filter(teryt_woj %in% szk$teryt_woj[[1]],
         typ_szk %in% szk$typ_szk[[1]])
```

W aneksie zaprezentowano tabelaryczne rozkłady wybranych wskaźników przedstawionych w raporcie, ale dla ogółu absolwentów, którzy w roku szkolnym `r rocznik` ukończyli szkoły policealne `r ifelse(woj$l_abs[[1]] < progLiczebnosci, paste0(""), paste0("w województwie, w którym znajduje się Państwa szkoła oraz "))`w całym kraju.
...
```{r}
if (woj$l_abs[[1]] < progLiczebnosci | woj$S3_12[[1]]$n < progLiczebnosci) {
  god2$S3_12[[1]] %>%
    as_tibble() %>% 
    select(n, tylko_ucz, ucz_prac, tylko_prac, bezrob, neet) %>% 
    mutate(across(n, ~linebreak(paste0("Absolwenci szkół\npolicealnych\nw całym kraju"), align = "l"))) %>%
    mutate(across(tylko_ucz:neet,
                  ~paste0(round(. * 100), "\\%"))) %>%
    kable(format = "latex", booktabs = TRUE, escape = FALSE, longtable = TRUE,
          linesep = "",
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = FALSE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>% 
    aadd_header_above(c(" ", "1. Tylko nauka" = 1, "2. Nauka i praca" = 1, "3. Tylko praca" = 1, "4. Bezrobocie" = 1, "5. Brak danych\no aktywności" = 1),
                     align = c("l", "c", "c", "c", "c", "c"))
} else if (woj$l_abs[[1]] >= progLiczebnosci & woj$S3_12[[1]]$n >= progLiczebnosci) {
  ...
  rbind(tabela_woj, tabela_kraj) %>% 
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = FALSE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>% 
    add_header_above(c(" ", "1. Tylko nauka" = 1, "2. Nauka i praca" = 1, "3. Tylko praca" = 1, "4. Bezrobocie" = 1, "5. Brak danych\no aktywności" = 1),
                     align = c("l", "c", "c", "c", "c", "c"))
}
```

  b) w poszczególnych zawodach - tylko na poziomie całego kraju
...
```{r}
if (woj$l_abs[[1]] < progLiczebnosci | woj$E2_nauka_kontyn[[1]]$n < progLiczebnosci) {
...
} else if (woj$l_abs[[1]] >= progLiczebnosci & woj$E2_nauka_kontyn[[1]]$n >= progLiczebnosci) {
  cat("  a) województwo ",
      woj$odmiany[[1]]$nazwa,
      "\n\n",
      sep = "")
...
```{r}
if (woj$l_abs[[1]] < progLiczebnosci | woj$Z4_ucz[[1]]$n < progLiczebnosci) {
  cat("Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów szkół policealnych z całego kraju.\n\n",
  ...
  tabelka = tabelka %>% 
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = FALSE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>%
    add_header_above(c(" ", "Absolwenci szkół policealnych\nkontynuujący naukę" = 1, "Absolwenci szkół policealnych\nniekontynuujący nauki" = 1),
                     align = c("l", "c", "c"))
...
if (woj$l_abs[[1]] >= progLiczebnosci & woj$Z4_ucz[[1]]$n >= progLiczebnosci) {
  cat("  a) województwo ",
      woj$odmiany[[1]]$nazwa,
      "\n\n* Część przepracowanego okresu - procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów szkół policealnych z województwa ",
      woj$odmiany[[1]]$nazwa_dop,
      "\n\n",
      sep = "")
  ...
  cat("\n\n* Formy wykonywania pracy w grudniu ",
      params$rok,
      " roku - procentowy udział grup wyróżnionych ze względu na formę wykonywania pracy wśród pracujących absolwentów szkół policealnych z województwa ",
      woj$odmiany[[1]]$nazwa_dop,
      "\n\n",
      sep = "")
  ...
  cat("\n\n  b) cały kraj\n\n* Część przepracowanego okresu - procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów szkół policealnych z całego kraju.\n\n")
  ...
  cat("\n\n* Formy wykonywania pracy w grudniu ",
      params$rok,
      " roku - procentowy udział grup wyróżnionych ze względu na formę wykonywania pracy wśród pracujących absolwentów szkół policealnych z całego kraju.\n\n",
      sep = "")
...
```{r}
if (woj$l_abs[[1]] < progLiczebnosci | sum(woj$B2_bezrob[[1]]$n) < progLiczebnosci) {
  cat("Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli zarejestrowani jako bezrobotni, wśród absolwentów szkół policealnych z całego kraju.")
  
  god2$B2_bezrob[[1]]$l_mies_bezrob[1] = "0 (brak bezrobocia)"
  
  tabelka = god2$B2_bezrob[[1]] %>% 
    as_tibble() %>% 
  ...
  cat(tabelka)
} else if (woj$l_abs[[1]] >= progLiczebnosci & sum(woj$B2_bezrob[[1]]$n) >= progLiczebnosci) {
  cat("  a) województwo ",
      woj$odmiany[[1]]$nazwa,
      "\n\nProcentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli zarejestrowani jako bezrobotni, wśród absolwentów szkół policealnych z województwa ",
      woj$odmiany[[1]]$nazwa_dop,
      ".\n\n",
      sep = "")
