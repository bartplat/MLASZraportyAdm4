...
if (szk$S3_12[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    "% absolwentów Państwa technikum pracowało^[Na podstawie umowy o pracę, umowy zlecenia lub prowadząc własną działalność gospodarczą.], ale nie kontynuowało nauki. Do grupy pracujących, ale nie kontynuujących edukacji formalnej należało ",
    ...
    "% absolwentów Państwa technikum oraz ",
...
# 6. Aneks: wyniki dla absolwentów techników `r ifelse(woj$l_abs[[1]] < progLiczebnosci, paste0(""), paste0("z województwa i", "\u00A0"))`z`r paste0("\u00A0")`całego kraju
...
W aneksie zaprezentowano tabelaryczne rozkłady wybranych wskaźników przedstawionych w raporcie, ale dla ogółu absolwentów, którzy w roku szkolnym `r rocznik` ukończyli technika `r ifelse(woj$l_abs[[1]] < progLiczebnosci, paste0(""), paste0("w województwie, w którym znajduje się Państwa szkoła oraz "))`w całym kraju.
...
if (woj$l_abs[[1]] < progLiczebnosci | woj$S3_12[[1]]$n < progLiczebnosci) {
  ...
  tabela_woj = woj$S3_12[[1]] %>% 
    as_tibble() %>% 
    select(n, tylko_ucz, ucz_prac, tylko_prac, bezrob, neet) %>% 
    mutate(across(n, ~linebreak(paste0("Absolwenci\ntechników\nw województwie\n",
...
if (nrow(tabela) > 0) {
  ...
  tabela %>% 
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = TRUE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>% 
    add_header_above(c("Zawody, w których obecni absolwenci\nuczyli się w technikach w całym kraju:", "1" = 1, "2" = 1, "3" = 1, "4" = 1, "5" = 1),
                     align = c("l", "c", "c", "c", "c", "c")) %>%
    add_header_above(c(" ",
                       "1. Tylko nauka\n2. Nauka i praca\n3. Tylko praca\n4. Bezrobocie\n5. Brak danych o aktywności" = 5),
                     align = c("c", "l")) %>% 
...
if (woj$l_abs[[1]] < progLiczebnosci | woj$Z4_ucz[[1]]$n < progLiczebnosci) {
  cat("Procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów techników z całego kraju.\n\n",
  ...
  tabelka = tabelka %>% 
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = FALSE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>%
    add_header_above(c(" ", "Absolwenci techników\nkontynuujący naukę" = 1, "Absolwenci techników\nniekontynuujący nauki" = 1),
                     align = c("l", "c", "c"))
...
if (woj$l_abs[[1]] >= progLiczebnosci & woj$Z4_ucz[[1]]$n >= progLiczebnosci) {
  cat("  a) województwo ",
      woj$odmiany[[1]]$nazwa,
      "\n\n* Część przepracowanego okresu - procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów techników z województwa ",
      woj$odmiany[[1]]$nazwa_dop,
      "\n\n",
      sep = "")
  ...
  cat("\n\n* Formy wykonywania pracy w grudniu ",
      params$rok,
      " roku - procentowy udział grup wyróżnionych ze względu na formę wykonywania pracy wśród pracujących absolwentów techników z województwa ",
      woj$odmiany[[1]]$nazwa_dop,
      "\n\n",
      sep = "")
  ...
  cat("\n\n  b) cały kraj\n\n* Część przepracowanego okresu - procentowy udział grup wyróżnionych ze względu na część przepracowanego okresu wśród absolwentów techników z całego kraju.\n\n")
  ...
  cat("\n\n* Formy wykonywania pracy w grudniu ",
      params$rok,
      " roku - procentowy udział grup wyróżnionych ze względu na formę wykonywania pracy wśród pracujących absolwentów techników z całego kraju.\n\n",
      sep = "")
...
if (woj$l_abs[[1]] < progLiczebnosci | sum(woj$B2_bezrob[[1]]$n) < progLiczebnosci) {
  cat("Procentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli zarejestrowani jako bezrobotni, wśród absolwentów techników z całego kraju.")
  ...
  cat("  a) województwo ",
      woj$odmiany[[1]]$nazwa,
      "\n\nProcentowy udział grup wyróżnionych ze względu na część okresu, w ciągu którego byli zarejestrowani jako bezrobotni, wśród absolwentów techników z województwa ",
      woj$odmiany[[1]]$nazwa_dop,
      ".\n\n",
      sep = "")
