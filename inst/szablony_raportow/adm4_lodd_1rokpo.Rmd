...
W porównaniu z raportami z wcześniejszych edycji monitoringu karier, zwiększono liczbę prezentowanych statusów opisujących  sytuację edukacyjno-zawodową absolwentów. Status "Brak nauki i brak pracy" rozdzielono na "Bezrobocie" oraz "Brak informacji o aktywności". Poniższa tabela przybliża konstrukcję poszczególnych statusów.
...
if (!is.na(szk$liczebnosc_dziedziny[[1]]$odsetek)) {
...
```{r, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, results='asis', fig.align=wyrownanieTabWykr}
if (!is.na(szk$liczebnosc_dyscypliny[[1]]$odsetek)) {
  if (round(szk$E2_nauka_kontyn[[1]]$studia * szk$E2_nauka_kontyn[[1]]$n) >= progLiczebnosci) {
...
# 6. Aneks: wyniki dla absolwentów liceów ogólnokształcących dla dorosłych `r ifelse(woj$l_abs[[1]] < progLiczebnosci, paste0(""), paste0("z województwa i "))`z całego kraju
...
if (woj$l_abs[[1]] < progLiczebnosci | woj$S3_12[[1]]$n < progLiczebnosci) {
  god2$S3_12[[1]] %>%
    ...
    kable_styling(position = wyrownanieTabWykr,
                  ...
                  full_width = FALSE,
                  repeat_header_text = "(ciąg dalszy tabeli)") %>% 
    add_header_above(c(" ", "1. Tylko nauka" = 1, "2. Nauka i praca" = 1, "3. Tylko praca" = 1, "4. Bezrobocie" = 1, "5. Brak danych o aktywności" = 1),
                     align = c("l", "c", "c", "c", "c", "c"))
